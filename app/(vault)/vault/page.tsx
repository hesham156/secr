          ...
          <Link className="rounded-lg border border-[var(--line)] bg-[var(--panel)] p-4 hover:border-[var(--accent)]" href={`/vault/${item.id}`} key={item.id}>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="mt-3 text-xs font-medium uppercase tracking-normal text-[var(--muted)]">{item.category}</p>
              </div>
              <ExternalLink className="h-4 w-4 text-[var(--muted)]" />
            </div>
            <div className="mt-4 flex flex-wrap gap-2 text-xs text-[var(--muted)]">
              <span className="rounded-md border border-[var(--line)] px-2 py-1">Strength: {passwordStrengthLabel(item.password)}</span>
              <span className="rounded-md border border-[var(--line)] px-2 py-1">Updated: {item.updatedAt}</span>
            </div>
          </Link>
        ))}
      </div>
